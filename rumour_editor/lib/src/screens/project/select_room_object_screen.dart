...
class _ObjectsListTile extends ConsumerWidget {
  ...
  Widget build(final BuildContext context, final WidgetRef ref) {
    ...
    return PlaySoundReferenceSemantics(
      ...
      child: value.when(
        data:
            (final objects) => ExpansionTile(
              initiallyExpanded:
                  objects
                      .where(
                        (final object) =>
                            object.id == roomObjectContext?.roomObject.id,
                      )
                      .isNotEmpty,
...
class _ZonesListView extends ConsumerWidget {
  /// Create an instance.
  const _ZonesListView({required this.selectObject, this.roomObjectContext});
    ...
    if (id == null) {
      return Cancel(
        child: SimpleScaffold(
          title: title,
          body: _ZonesListView(selectObject: selectObject),
        ),
      );
    }
