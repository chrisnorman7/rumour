...
/// Provides all quests.
@riverpod
Future<List<Quest>> quests(final Ref ref) async {
  final database = ref.watch(databaseProvider);
  final manager = database.managers.quests;
  final quests = await manager
      .orderBy(
        (final o) => o.description.asc(),
      )
      .get();
  if (quests.isEmpty) {
    quests.add(
      await manager.createReturning(
        (final o) => o(
          name: 'Autogenerated quest',
          description: 'A quest which was automatically created by the editor',
        ),
      ),
    );
  }
...
@riverpod
Future<List<QuestStage>> questStages(final Ref ref, final int questId) async {
  final database = ref.watch(databaseProvider);
  final manager = database.managers.questStages;
  final stages = await manager
      .filter((final f) => f.questId.id.equals(questId))
      .orderBy(
        (final o) => o.label.asc(),
      )
      .get();
